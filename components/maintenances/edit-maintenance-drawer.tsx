...
export function EditMaintenanceDrawer({ maintenance, open, onOpenChange, onSuccess }: EditMaintenanceDrawerProps) {
  ...
  useEffect(() => {
    if (open) {
      reset({
        ...
        deliveryDate: maintenance.deliveryDate ? new Date(maintenance.deliveryDate).toISOString().split("T")[0] : "",
  ...
  const onSubmit = async (data: MaintenanceFormData) => {
    if (data.status === "CONCLUIDA" && !data.deliveryDate) {
      alert("Data de entrega é obrigatória para manutenções concluídas.")
      return
    }
